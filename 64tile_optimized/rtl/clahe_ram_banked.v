...
module clahe_ram_banked #(
    ...
    // Fix: Pipeline alignment for Hist/CDF Crossbar (Same as Mapping)
    // RAM Read Latency = 1 cycle. Mux Select must be delayed 1 cycle.
    reg [1:0] curr_hist_rd_bank_d1;
    reg [1:0] curr_cdf_bank_d1;

    always @(posedge pclk or negedge rst_n) begin
        if (!rst_n) begin
            curr_hist_rd_bank_d1 <= 2'd0;
            curr_cdf_bank_d1     <= 2'd0;
        end else begin
            curr_hist_rd_bank_d1 <= curr_hist_rd_bank;
            curr_cdf_bank_d1     <= curr_cdf_bank;
        end
    end

    // 1. Hist Read Data (From Active Set, Active Bank)
    // Use delayed Bank Selection to match RAM Latency
    wire [15:0] hist_rd_data_raw = (ping_pong_flag == 0) ?
         ram_dout[0][curr_hist_rd_bank_d1] : ram_dout[1][curr_hist_rd_bank_d1];

    // 2. CDF Read Data (From Active Set, Active Bank)
    wire [15:0] cdf_rd_data_raw = (ping_pong_flag == 0) ?
         ram_dout[0][curr_cdf_bank_d1] : ram_dout[1][curr_cdf_bank_d1];
    ...
    always @(*) begin
        ...
        case (get_bank_id(
            ...
            2'd0: mapping_tl_rd_data = ram_dout[inactive_set][0][7:0];
            2'd1: mapping_tl_rd_data = ram_dout[inactive_set][1][7:0];
            2'd2: mapping_tl_rd_data = ram_dout[inactive_set][2][7:0];
            2'd3: mapping_tl_rd_data = ram_dout[inactive_set][3][7:0];
        endcase
        ...
        case (get_bank_id(
            ...
            2'd0: mapping_tr_rd_data = ram_dout[inactive_set][0][7:0];
            2'd1: mapping_tr_rd_data = ram_dout[inactive_set][1][7:0];
            2'd2: mapping_tr_rd_data = ram_dout[inactive_set][2][7:0];
            2'd3: mapping_tr_rd_data = ram_dout[inactive_set][3][7:0];
        endcase

        // BL
        case (get_bank_id(
            mapping_bl_tile_idx_d1
        ))  // Use delayed idx
            2'd0: mapping_bl_rd_data = ram_dout[inactive_set][0][7:0];
            2'd1: mapping_bl_rd_data = ram_dout[inactive_set][1][7:0];
            2'd2: mapping_bl_rd_data = ram_dout[inactive_set][2][7:0];
            2'd3: mapping_bl_rd_data = ram_dout[inactive_set][3][7:0];
        endcase
        ...
        case (get_bank_id(
            ...
            2'd0: mapping_br_rd_data = ram_dout[inactive_set][0][7:0];
            2'd1: mapping_br_rd_data = ram_dout[inactive_set][1][7:0];
            2'd2: mapping_br_rd_data = ram_dout[inactive_set][2][7:0];
            2'd3: mapping_br_rd_data = ram_dout[inactive_set][3][7:0];
        endcase
    end
