...
## 5. 关键机制：流水线除法器 (Pipelined Divider)

### 5.1 问题背景：组合除法的时序灾难

**理论来源**: *Parhi Chapter 2 (Iteration Bound and Critical Path)*

CDF 归一化公式：
$$normalized\_cdf = \frac{(cdf - cdf_{min}) \times 255}{cdf_{max} - cdf_{min}}$$

Verilog 中的 `/` 运算符被综合为 **迭代减法器**，形成超长组合逻辑链：

```
Baseline 关键路径分析 (来自 Vivado Timing Report):
- 逻辑级数: 185 级 (159 × CARRY4 + 18 × LUT3 + 7 × LUT2 + 1 × LUT1)
- 数据路径延迟: 35.546 ns
- 理论最高频率: 1 / 35.546ns ≈ 28 MHz
```

### 5.2 Cut-Set Pipelining 变换

**理论来源**: *Parhi Chapter 2.3 (Pipelining of FIR Filters)*

Cut-Set 变换的核心思想是：在数据流图 (DFG) 中选择一个 **割集 (Cut-Set)**，在割集上的所有边插入等量延迟，保持功能不变的同时缩短关键路径。

```
原始 DFG:
  norm_stage2_mult ──────────────────▶ [DIVIDER] ──────────────▶ norm_result
                                         (35ns)

Cut-Set 变换后:
  norm_stage2_mult ──▶ [REG] ──▶ [S0] ──▶ [REG] ──▶ [S1] ──▶ ... ──▶ [S32] ──▶ [REG] ──▶ norm_result
                        │                  │                          │
                       D=1                D=1         ...            D=1
                              每级延迟 < 1ns
```
...
**理论来源**: *Parhi Chapter 14 (Arithmetic Operations)*

我们采用 **Non-Restoring Division** 算法，天然适合流水线化：
...
// 33级流水线架构:
// Stage 0:      输入锁存
// Stages 1-32:  每级处理 1 bit (减法/移位/试商)
// 总延迟:       33 周期
// 吞吐量:       1 result/cycle (流水线填满后)
```

### 5.4 Retiming：地址延迟对齐

**理论来源**: *Parhi Chapter 4 (Retiming)*

Retiming 定理指出：在保持输入输出延迟总和不变的前提下，可以任意移动寄存器位置以优化关键路径。

**问题**: 除法器引入 33 周期延迟，地址路径必须有相同延迟才能对齐。

**解决方案**: 插入 33 级地址移位寄存器

```verilog
// clahe_clipper_cdf.v 中的延迟对齐
...
always @(posedge pclk or negedge rst_n) begin
    ...
    else if (state == WRITE_LUT) begin
        addr_delay_reg[0] <= norm_stage2_addr;
...
| 指标 | Baseline (组合除法) | Optimized (流水线除法) |
|------|---------------------|------------------------|
| 关键路径延迟 | 35.5 ns | 6.7 ns |
| 逻辑级数 | 185 | 5 |
| WNS @ 74MHz | -22.3 ns (Fail) | +6.8 ns (Met) |
| WNS @ 100MHz | N/A | +3.2 ns (Met) |
...
通过 **Folding (折叠)** 减少了资源，通过 **Interleaving (交织)** 解决了冲突，通过 **Pipelining (流水线)** 提升了频率，通过 **Retiming** 实现了延迟对齐。这证明了扎实的理论基础是解决复杂工程问题的关键。
